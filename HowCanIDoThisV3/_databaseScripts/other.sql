...
AS $procedure$
	...
	begin
		...
		if (SELECT count(id) FROM chats where fkUserFirst = userid or fkUserSecond = userid) != 0 then
			...
			update chats set fkLastMessage = tmpMessageId where id = tmpChatId;
		else
...
AS $function$
	begin
		RETURN QUERY SELECT t1.id, t1.fkUserSecond, t2.name, t2.profileImg, t3.messageText, t3.sendTime 
			...
			on t1.fkLastMessage = t3.id WHERE t1.fkUserFirst = _userId union SELECT t1.id, t1.fkUserFirst, t2.name, t2.profileImg, t3.messageText, t3.sendTime 
			FROM Chats as t1 LEFT JOIN Users as t2 on t1.fkUserFirst = t2.id LEFT JOIN Messages as t3
			on t1.fkLastMessage = t3.id WHERE t1.fkUserSecond = _userId;
	END;
$function$
;
