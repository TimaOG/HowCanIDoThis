...
func ordersListWithoutSorting() ViewData {
	...
	sqlRequest := `SELECT t1.id, t1.name, t1.fkUserOwner, t2.name, t1.discribtion ,t1.price, t1.deadline, t1.urgency, t1.workType, t1.tags FROM Orders as t1 JOIN Users as t2 on t1.FkUserOwner = t2.id WHERE t1.isActive = true LIMIT 20;`
...
func getSelectWithFilters(sortFields SortFields) string {
	sqlRequest := `SELECT t1.id, t1.name, t1.fkUserOwner, t2.name, t1.discribtion ,t1.price, t1.deadline, t1.urgency, t1.workType, t1.tags FROM Orders as t1 JOIN Users as t2 on t1.FkUserOwner = t2.id WHERE t1.isActive = true `
...
func getOrderById(id uint32) OrderById {
	sqlRequest := `SELECT t1.id, t1.name, t1.fkUserOwner, t2.name, t1.discribtion ,t1.price, t1.deadline, t1.urgency, t1.workType, t1.tags, t1.tzPath FROM Orders as t1 JOIN Users as t2 on t1.FkUserOwner = t2.id WHERE t1.id = $1;`
	db, _ := sql.Open("postgres", dbURL)
	var order OrderById
