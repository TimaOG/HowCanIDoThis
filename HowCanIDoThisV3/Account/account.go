...
func NewOrderPage(w http.ResponseWriter, r *http.Request) {
	if !auth.CheckUserAuth(w, r) {
		//http.Redirect(w, r, fmt.Sprintf(config.SiteAdress+"login?target=account/newOrder"), http.StatusSeeOther)
		w.Write([]byte("{\"status\": \"notAuth\", \"body\": \"1\"}"))
		return
	}
	auth.RefreshCookie(w, r)
...
func NewOfferPage(w http.ResponseWriter, r *http.Request) {
	if !auth.CheckUserAuth(w, r) {
		//http.Redirect(w, r, fmt.Sprintf(config.SiteAdress+"login?target=account/newOffer"), http.StatusSeeOther)
		w.Write([]byte("{\"status\": \"notAuth\", \"body\": \"1\"}"))
		return
	}
	auth.RefreshCookie(w, r)
...
func AccountDeletePage(w http.ResponseWriter, r *http.Request) {
	if !auth.CheckUserAuth(w, r) {
		//http.Redirect(w, r, fmt.Sprintf(config.SiteAdress+"login?target=account/delete"), http.StatusSeeOther)
		w.Write([]byte("{\"status\": \"notAuth\", \"body\": \"1\"}"))
		return
	}
	if r.Method == http.MethodPost {
