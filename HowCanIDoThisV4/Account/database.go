...
func getUserByIdFull(id uint32) ViewData {
	sqlRequest := `SELECT username, email, discribtion, profileImg, rating, balance ,isPremiumUser, isActiveUser, 
	...
	checkError(err)
	sqlRequest = `SELECT id, ordername, discribtion, price FROM Orders WHERE fkUserOwner = $1;`
	...
	sqlRequest = `SELECT id, offername, price, isActive, coverPath FROM Offers WHERE fkUserOwner = $1;`
	...
	sqlRequest = `SELECT t4.id, t4.ordername, t4.discribtion, t4.price, t2.username, t2.id, t3.username, t3.id, 
	...
	sqlRequest = `SELECT t4.id, t4.ordername, t4.price, t2.username, t2.id, t3.username, t3.id, 
	t1.startTime, t4.deadline FROM Mathes as t1 LEFT JOIN Users as t2 on t1.fkUserTaskOwner = t2.id 
	LEFT JOIN Users as t3 on t1.fkUserWhoDo = t3.id LEFT JOIN Orders as t4 on t1.fkWhatTaskId = t4.id
...
func getOfferById(id uint32) []byte {
	sqlRequest := `SELECT offername, discribtion, price, daysToComplite, workType, taskTypeFirst, taskTypeSecond, tags FROM Offers WHERE id = $1;`
	db, _ := sql.Open("postgres", dbURL)
	var offer OfferById
	res := db.QueryRow(sqlRequest, id)
...
func addOrder(order Order) uint32 {
	sqlRequest := `INSERT INTO Orders (ordername, fkUserOwner, discribtion, price, deadline, urgency, workType,
...
func addOffer(offer Offer) uint32 {
	sqlRequest := `INSERT INTO Offers (offername, fkUserOwner, discribtion, price, daysToComplite, workType,
...
func changeOffer(offer OfferById, offerId uint32, fn string, userId uint32) {
	...
	db, _ := sql.Open("postgres", dbURL)
	if fn == "" {
		sqlRequest = `UPDATE Offers SET ordername = $1, discribtion = $2, price = $3, daysToComplite = $4, workType = $5,
		...
		sqlRequest = `UPDATE Offers SET offername = $1, discribtion = $2, price = $3, daysToComplite = $4, workType = $5,
...
func getUserPasswordById(id uint32) string {
	sqlRequest := `SELECT userpassword FROM Users WHERE id = $1;`
	db, _ := sql.Open("postgres", dbURL)
...
func saveNewPassword(id uint32, newPassword []byte) {
	sqlRequest := `UPDATE Users SET userpassword = $1 WHERE id = $2`
	db, _ := sql.Open("postgres", dbURL)
...
func getThemeNumber(userId uint32) uint8 {
	sqlRequest := `SELECT theme from Users where id = $1`
	db, _ := sql.Open("postgres", dbURL)
	var themeId uint8
	res := db.QueryRow(sqlRequest, userId)
	err := res.Scan(&themeId)
	checkError(err)
	defer db.Close()
	return themeId
}

func changeThemeNumber(userId uint32, themeNumber uint8) {
	if themeNumber == 0 || themeNumber == 1 || themeNumber == 2 {
		sqlRequest := `update users set theme = $1 where id = $2`
		db, _ := sql.Open("postgres", dbURL)
		_, err := db.Exec(sqlRequest, themeNumber, userId)
		checkError(err)
		defer db.Close()
	}
}
