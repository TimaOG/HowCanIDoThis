...
func startServerAndAddHandles() {
	...
	r.HandleFunc("/account/getThemeNumber", account.GetThemeNumberPage)
	r.HandleFunc("/account/changeThemeNumber/{themeNumber:[0-2]}", account.ChangeThemeNumberPage)
	...
	r.HandleFunc("/chat/ws", chat.ChatPageWsId)
	r.HandleFunc("/chat/saveChatFile", chat.ChatSaveFile)
	r.HandleFunc("/chat/makeMatch", chat.MakeMatchPage)
