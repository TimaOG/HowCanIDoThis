...
func offersListWithoutSorting() ViewData {
	...
	sqlRequest := `SELECT t1.id, t1.offername, t1.fkUserOwner, t2.username, t2.isPremiumUser ,t1.coverPath, t1.price, t1.daysToComplite, t1.workType, t1.tags, t1.rating, t1.historyCount FROM Offers as t1 LEFT JOIN Users as t2 on t1.fkUserOwner = t2.id WHERE t1.isActive = true LIMIT 18;`
...
func getSelectWithFilters(sortFields SortFields) string {
	sqlRequest := `SELECT t1.id, t1.offername, t1.fkUserOwner, t2.username, t2.isPremiumUser ,t1.coverPath, t1.price, t1.daysToComplite, t1.workType, t1.tags, t1.rating, t1.historyCount FROM Offers as t1 JOIN Users as t2 on t1.fkUserOwner = t2.id WHERE t1.isActive = true `
	...
	sqlRequest += "ORDER BY t1.price " + sortFields.OrderBy + " LIMIT 18 OFFSET " + fmt.Sprint(offset*18)
	return sqlRequest
}

func getOfferById(id uint32) OfferById {
	sqlRequest := `SELECT t1.id, t1.offername, t1.discribtion ,t1.fkUserOwner, t2.username, t2.isPremiumUser ,t1.coverPath, t1.price, t1.daysToComplite, t1.workType, t1.tags, t1.rating, t1.historyCount FROM Offers as t1 LEFT JOIN Users as t2 on t1.fkUserOwner = t2.id WHERE t1.id = $1;`
	db, _ := sql.Open("postgres", dbURL)
	var offer OfferById
